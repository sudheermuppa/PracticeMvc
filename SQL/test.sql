...
DECLARE
  ...
  TYPE T_ROW_ID IS TABLE OF VARCHAR2(CHAR 15);
  TYPE T_MEM_NUM IS TABLE OF VARCHAR2(CHAR 30);
  TYPE T_PROG_NAME IS TABLE OF VARCHAR2(CHAR 50);
  TYPE T_MEMBER_MEM_NUM IS TABLE OF VARCHAR2(CHAR 250);
  ...
  CURSOR C_MEMBERS IS
   SELECT ROW_ID, MEM_NUM, PROG_NAME, MEMBER_MEM_NUM
      ...
      LEFT OUTER JOIN SIEBEL.S_LOY_PROGRAM SBL_PROG ON (SBL_MEM.PROGRAM_ID = SBL_PROG.ROW_ID AND SBL_PROG.NAME = IN_PROG_NAME)
       WHERE
      STG_TXN.REC_STATUS = 'Prestaged' AND STG_TXN.FILE_ID = IN_FILE_ID  );
	 ...
	 LOOP	
	 	...
	 	FORALL i IN V_ROW_ID.FIRST..V_ROW_ID.LAST
	 	 UPDATE SIEBEL._LM_<PrestageTable>
	         SET
	         ERROR_CODE =
	                DECODE(V_MEM_NUM(i), NULL, 'SBL-FINT-0007', ERROR_CODE),
	         ERROR_DESC =
	                DECODE(V_MEM_NUM(i), NULL, LM_ERROR('SBL-FINT-0007',V_MEMBER_MEM_NUM(i)) , ERROR_DESC),
	        REC_STATUS =
	                DECODE(V_MEM_NUM(i), NULL, 'Error', REC_STATUS)
	         --_LM_<PiggyBackUpdate>
